...
public sealed class CommonSecretsDiagnosticCheck : ILeveledDiagnosticCheck
{
    ...
    public async Task<DiagnosticResult> RunAsync(CancellationToken cancellationToken = default)
    {
        ...
        {
            stopwatch.Stop();
            return new DiagnosticResult(Name, DiagnosticStatus.Warning, "No secret provider health probes are registered.", stopwatch.Elapsed);
        }
        ...
        if (unavailable.Length > 0)
        {
            ...
            return new DiagnosticResult(Name, DiagnosticStatus.Unhealthy, $"Secret provider health failed: {names}.", stopwatch.Elapsed);
        }

        if (enabled.Length == 0)
        {
            return new DiagnosticResult(Name, DiagnosticStatus.Warning, "All registered secret providers are disabled.", stopwatch.Elapsed);
        }

        string healthyNames = string.Join(", ", enabled.Select(result => result.ProviderName));
        return new DiagnosticResult(Name, DiagnosticStatus.Healthy, $"Secret providers healthy: {healthyNames}.", stopwatch.Elapsed);
    }
}
